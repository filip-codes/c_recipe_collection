...
int show_search_by_recipe_name_with_ncurses() {
    char *p_recipe_prompt = malloc(sizeof(char));
    if (p_recipe_prompt == NULL)
        exit(0);

    strcpy(p_recipe_prompt, "");
    ...
    echo();
    mvwprintw(g_window, 1, 1, "Search by recipe name:");
    mvwgetstr(g_window, 2, 1, p_recipe_prompt);

    int recipes_count = 0;
    struct recipe *p_head = g_recipe;
    struct recipe *tmp_recipe = NULL;
    while(p_head != NULL) {
        if (strstr(p_head->p_name, p_recipe_prompt) != NULL) {
